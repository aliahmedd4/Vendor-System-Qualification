...
| DEV-009 | IQ-04 | `[CONFIRMED]` | Documented container image `jenspiegsa/openclinica:3.14` was **not pullable** (wrong Docker Hub namespace/tag; `jenspiegsa` is the GitHub author, the Hub image is `piegsaj`). | Install blocked; no patient-safety/data-integrity impact — caught at IQ before any data. | Corrected | Compose updated to the real image `piegsaj/openclinica:oc-3.13` (PostgreSQL 9.5, context path `/OpenClinica`, default admin root/12345678); digest to be pinned at IQ-04. Demonstrates why IQ verifies the *actual* installed artefact rather than trusting the spec. | — |
| DEV-010 | OQ-01, OQ-02, OQ-06 | `[CONFIRMED]` | Automated login click timed out after 15 s. Selector `input[name="Submit"], button[type="submit"]` resolved to the **hidden** `value="Submit Password Request"` control belonging to the forgot-password form that shares the OpenClinica login page. | **Test script defect** — no patient-safety/data-integrity impact; the application behaves correctly. | Login selectors matched the wrong of two `name="Submit"` controls on the page. | Login selectors scoped to the login form (`form:has(input[name="j_password"])`); post-login assertion on an authenticated-only element added; affected tests re-executed. **Not a finding against OpenClinica.** |
| DEV-011 | OQ-01, OQ-02, OQ-12, OQ-15, OQ-28 | `[CONFIRMED]` | Harness DB connections failed: `ECONNREFUSED ::1:5432` and `127.0.0.1:5432`. | **Environment defect** — the qualification environment, not the software. | To confirm at Stage 2: DB container port unpublished and/or Node resolving `localhost`→IPv6 `::1` while Postgres binds IPv4. | Publish `127.0.0.1:5432`; pin harness host to IPv4; add a fail-fast connectivity check in Playwright `globalSetup`. **Not a finding against OpenClinica.** |
| DEV-012 | OQ-01, OQ-02 | `[CONFIRMED]` | Login-audit query errored: `column "login_status" does not exist`. | **Test script defect** — no impact on the application; OpenClinica records logins correctly (`audit_user_login` had 6 rows). | Harness assumed a column name; the real OpenClinica 3.13 column is `login_status_code` (integer). | Query corrected to `login_status_code` after confirming against the live `information_schema`; timestamps emitted in UTC. **Not a finding against OpenClinica.** |
